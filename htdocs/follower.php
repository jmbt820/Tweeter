...
$user_id = get_get_data('follower_detail');
...
try {
  ...
  if (isset($_POST['update_follow']) === TRUE) {
    ...
    header('Location:' . ROOT_URL . '/follower.php?follower_detail=' . $user_id);
    exit;
  }
  ...
  $list_query = $dbh->prepare(
    ...
    JOIN profile_table ON follow_table.user_id = profile_table.user_id
    WHERE follow_table.follow_id = :user_id AND follow_table.status = 1
  ...
  $list_query->execute();
  $follower_data = $list_query->fetchAll();
  $list_query = NULL;
  ...
  $status_query = $dbh->prepare(
    ...
    $error_message = 'フォロワーを表示できませんでした。管理者に問い合わせてください。';
  }

  $follower_data = set_follow_status($follower_data, $status_data);
  $follower_data = change_array_entity($follower_data);
  $error_message = change_entity($error_message);

  include_once VIEW_DIRECTORY . '/follower.html';
